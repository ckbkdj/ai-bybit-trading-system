...
from core.providers.fomc_pit import FOMCPITStore, backfill_fomc_pit
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backfill strict-PIT FOMC statement event factors"
    )
    parser.add_argument("--start", type=_date, required=True)
    parser.add_argument("--end", type=_date, required=True)
    parser.add_argument(
        "--database", type=Path, default=ROOT / "data" / "macro_pit.sqlite3"
    )
    parser.add_argument(
        "--cache-dir", type=Path, default=ROOT / "data" / "fomc_pit_cache"
    )
    parser.add_argument(
        "--report",
        type=Path,
        default=ROOT / "model_results" / "evaluation" / "fomc_pit_backfill_report.json",
    )
    args = parser.parse_args()
    report = backfill_fomc_pit(
        FOMCPITStore(args.database),
