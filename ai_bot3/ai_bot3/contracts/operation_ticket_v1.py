...
TicketAction = Literal["OPEN", "INCREASE", "REDUCE", "CLOSE", "CANCEL", "REPLACE"]
...
class TicketProtection(ContractModel):
    ...
    @model_validator(mode="after")
    def close_fractions_are_valid(self):
...
class TicketEconomics(ContractModel):
    ...
    @model_validator(mode="after")
    def after_cost_is_consistent(self):
        computed = self.expected_return_bps - self.estimated_fee_bps - self.estimated_slippage_bps
        computed -= self.estimated_funding_bps + self.model_error_buffer_bps
        if abs(computed - self.expected_return_after_cost_bps) > 0.01:
