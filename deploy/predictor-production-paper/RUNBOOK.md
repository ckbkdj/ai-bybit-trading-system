...
3. Before first start, seed the approved shadow model/data bundle under the absolute `PREDICTOR_DATA_DIR`; runtime `data/`, `models/` and `model_results/` are created/read there, never in the code checkout.
4. Use exactly one publication database path: `FORECAST_PUBLICATION_OUTBOX_DB`. Predictor-realtime and publication-worker must point to the same local SQLite file.
5. Run `python scripts/validate_production_paper_deployment.py`. It must report `status=PASS`, `mainnet_allowed=false`, `real_capital_at_risk=false`.
6. Verify WireGuard, the mTLS chain, per-executor token map, the four distinct local data directories, disk quota and NTP.
7. The control plane must start as `python -m uvicorn api.control_plane_main:app --host 127.0.0.1 --port 8000`; the legacy `api_server.py` is not the production control-plane entrypoint.
